...
#
#         # Modify attrs to use the normalized username for validation
#         attrs['username'] = normalized_username
#
#         # Attempt to authenticate the user with normalized username and password
#         user = authenticate(username=normalized_username, password=password)
#
#         # Case 1: If user is not found (incorrect username)
#         if user is None:
#             # Check if the username exists in the database (case-insensitive search)
#             try:
#                 user = User.objects.get(username__iexact=normalized_username)  # Case-insensitive lookup
#                 # If user exists but password is incorrect, show password error
...
#                 "detail": f"Invalid role. This account is associated with the '{user_profile.role}' role."
#             })
#
#         # Add username and role to the response data
#         data = super().validate(attrs)  # Validate with the updated attrs
#         data['username'] = user.username  # Return the original case of username
...
class LoginTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def validate(self, attrs):
        ...
        if '@' in normalized_input:
            # Input is an email, try to find the user by email
            try:
            ...
            # Input is a username, try to find the user by username
            try:
        ...
        # Attempt to authenticate the user with the username or email and password
